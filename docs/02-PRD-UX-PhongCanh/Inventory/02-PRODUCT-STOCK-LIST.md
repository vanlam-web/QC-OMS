...
| Giá nhập cuối | Snapshot dùng cho PriceBook formula; không phải giá vốn kế toán |
...
| Sửa giá nhập cuối | Chỉ dùng tạm trước Purchase receipt; cần quyền sửa bảng giá/sản phẩm |
...
## 7.1. Giá nhập cuối tạm thời trước Purchase

Trước khi module Purchase receipt hoàn chỉnh, Product admin cho phép Owner/admin nhập hoặc sửa `Giá nhập cuối` (`latest_purchase_cost`) để PriceBook formula có dữ liệu chạy.

Quy tắc:

- Đây không phải giá vốn kế toán và không thay thế phiếu nhập.
- Field này có thể để trống/null; PriceBook formula tính như `0` nếu thiếu.
- Khi sửa thủ công, lưu `latest_purchase_cost_at` và audit tối thiểu người sửa/thời gian.
- Sau khi Purchase receipt `posted` được triển khai, phiếu nhập là nguồn chính cập nhật `latest_purchase_cost`; thao tác sửa tay chỉ còn dùng để hiệu chỉnh có kiểm soát nếu Owner cho phép.
