...
├──────────────────────────────────────────────────────────────────────────────┤
│ [Theo mã/tên hàng...] [Cách bán] [Trạng thái hàng] [Tạo CT cho bộ lọc]       │
│                                                                              │
...
Màn này có thao tác gắn công thức theo nhóm hàng/bộ lọc. Công thức chỉ lấy `giá nhập cuối`, sau đó tính `Chi phí`, `Lợi nhuận` và điều chỉnh theo từng bảng giá.

Giá cuối luôn làm tròn lên `1,000đ`. UI không cần hiển thị lựa chọn làm tròn.

Mỗi ô giá có một chế độ:

- `Giá tay`: người dùng nhập trực tiếp, không tự đổi theo giá nhập cuối.
- `Theo công thức`: ô giá nhớ công thức đang áp dụng và tự tính lại khi `giá nhập cuối` thay đổi.

Khi giá nhập cuối thay đổi, hệ thống tính lại các ô đang theo công thức và hiển thị chênh lệch để Owner dễ kiểm tra. Không cần bấm áp dụng lại chỉ vì giá nhập thay đổi. Nếu người dùng nhập giá trực tiếp vào ô đang theo công thức, ô đó chuyển về `Giá tay`. Nếu người dùng áp công thức khác, công thức mới thay công thức cũ.
...
```text
Áp dụng cho: Tên chứa Fomex, mã chứa F5, cách bán tấm
...
```text
[Tên chứa: Fomex] [Mã chứa: F5] [Cách bán: tấm] [Trạng thái: active]
...
- Slice đầu chưa có `product_groups`, nên chưa có filter nhóm hàng trong formula MVP.
- Tên hàng hoặc mã hàng là điều kiện chính trong slice đầu.
- Nên kết hợp tên/mã với `sell_method` để tránh bắt nhầm sản phẩm.
- Khi module nhóm hàng có schema chuẩn, bổ sung filter nhóm hàng sau.
- Nếu cần rất đặc biệt, cho phép gắn công thức trực tiếp cho một vài sản phẩm.
- Khi sản phẩm mới được tạo và khớp bộ lọc công thức, lần gắn/chạy công thức sau sẽ đưa sản phẩm đó vào danh sách xem trước.
...
Trước khi gắn công thức hàng loạt, màn preview phải hiển thị giá hiện tại, chế độ hiện tại (`Giá tay` hoặc `Theo công thức`), giá nhập cuối, chi phí, lợi nhuận, điều chỉnh bảng giá, giá mới sau làm tròn và chênh lệch.

Sau khi bấm `Gắn công thức`, các ô được chọn sẽ theo công thức đó cho tới khi người dùng nhập giá tay hoặc gắn công thức khác.
...
- có nút gắn công thức theo bộ lọc khi Owner chủ động dùng
- phân biệt rõ ô đang là `Giá tay` hay `Theo công thức`
