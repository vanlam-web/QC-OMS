...
| `id` | `uuid` | ❌ | Khóa chính |
| `organization_id` | `uuid` | ❌ | FK → `public.organizations.id` |
| `customer_id` | `uuid` | ✅ | FK → `public.customers.id`; null nếu khách lẻ nợ |
...
| `balance_after_customer` | `numeric(12,0)` | ✅ | Tổng nợ khách sau giao dịch; null nếu là khách lẻ nợ chưa gán hồ sơ |
...
| `retail_debt_note` | `text` | ✅ | Ghi chú nhận diện nếu là khách lẻ nợ |
...
- Nếu `customer_id` null và `entry_type = 'invoice_debt'`, `retail_debt_note` bắt buộc.
- Nếu `customer_id` không null, `balance_after_customer` bắt buộc.
