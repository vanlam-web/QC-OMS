...
- mở lại báo giá vào POS như một nháp local để sửa, lưu thành báo giá mới hoặc checkout như nháp bình thường
...
### BR-QUOTE-04: Mở báo giá thành nháp rồi xử lý như POS draft

Khi khách đồng ý hoặc khi nhân viên muốn dùng lại nội dung báo giá cũ, báo giá được mở lại thành đơn nháp local để kiểm tra/sửa.

Từ nháp đó:

- bấm `[THANH TOÁN]` tạo hóa đơn bán hàng với mã `HD...` như checkout POS bình thường
- bấm `[BÁO GIÁ]` tạo báo giá mới độc lập với mã `BG...` mới
- bấm `X`/đóng tab thì không ghi gì thêm

Báo giá gốc không bị đổi trạng thái, không bị khóa checkout lại và không cần lưu quan hệ nguồn vào hóa đơn trong Phase 3A.
...
Trạng thái báo giá MVP chỉ cần đơn giản:

| Trạng thái | Ý nghĩa | Phase |
|---|---|---|
| `active` | Báo giá đã lưu, còn xem/mở lại được | Phase 3A |
| `cancelled` | Nếu sau này cần hủy báo giá thủ công | Future/optional |
...
Không làm nút `Sao chép báo giá` riêng. Nếu muốn dùng lại nội dung báo giá cũ, nhân viên mở báo giá vào POS draft rồi lưu báo giá mới hoặc thanh toán từ nháp đó.

### BR-QUOTE-07: Báo giá đã lưu không sửa đè

Báo giá đã lưu không bị sửa đè trực tiếp.

Khi mở lại báo giá và nhân viên bấm `[BÁO GIÁ]` sau khi sửa, hệ thống tạo một báo giá mới độc lập:

```text
BG000123 -> mở vào POS draft -> lưu BG000124
```

Báo giá mới không phải revision, không giữ `base_code` của báo giá cũ và không làm báo giá cũ bị hủy/converted.

Trong Phase 3A, audit tối thiểu là `created_by`, `created_at` và trạng thái của từng báo giá độc lập.
...
### BR-INV-03: Hóa đơn từ nháp mở lại vẫn là hóa đơn POS bình thường

Nếu hóa đơn được tạo sau khi mở báo giá vào POS draft, hóa đơn vẫn là hóa đơn checkout bình thường.

Báo giá gốc đã là chứng từ riêng trong Sales Documents nên không cần bắt buộc lưu `source_quote_id/source_quote_code` trên hóa đơn trong Phase 3A.
...
6. Thanh toán từ nháp mở lại sinh hóa đơn `HD...` như checkout POS bình thường.
7. Báo giá và hóa đơn bán hàng đều giữ snapshot dòng hàng tại thời điểm lưu.
8. Sửa/lưu lại từ nháp mở báo giá tạo báo giá mới độc lập, không ghi đè báo giá cũ và không tạo revision `BG000123.01`.
