  ...
  return (
    <section aria-label="Khách hàng" className="customer-panel">
      <header>
        <h2>Khách hàng</h2>
        {selectedCustomer ? (
          <button type="button" onClick={() => onSelectCustomer(null)}>
            Bỏ chọn
          </button>
        ) : null}
      </header>
      ...
      {selectedCustomer ? (
        <p>Đã chọn {selectedCustomer.code} - {selectedCustomer.name}</p>
      ) : (
      ...
      <form aria-label="Tìm khách hàng" className="customer-search" onSubmit={searchCustomers}>
        ...
        <button disabled={busy} type="submit">Tìm khách</button>
      </form>
      ...
      <form aria-label="Tạo khách nhanh" className="customer-create" onSubmit={createCustomer}>
        <label>
          Mã khách
        ...
        <button disabled={busy} type="submit">Tạo khách</button>
      </form>
    </section>
  )
}
